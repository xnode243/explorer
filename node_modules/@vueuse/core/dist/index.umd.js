(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.VueUse = {}, global.VueDemi, global.VueUseShared));
  ...
  function useCssVar(prop, el, { window = defaultWindow } = {}) {
      if (!window)
          return vueDemi.ref('');
      const varRef = vueDemi.ref('');
      const elRef = vueDemi.ref(vueDemi.unref(el) || window.document.documentElement);
      shared.tryOnMounted(() => {
          varRef.value = window.getComputedStyle(elRef.value).getPropertyValue(prop);
      });
      vueDemi.watch(varRef, (val) => {
          var _a;
  ...
  ***************************************************************************** */
  ...
  function useResizeObserver(target, callback, options = {}) {
      ...
      const stopWatch = vueDemi.watch(targetRef, (newValue) => {
          cleanup();
  ...
  function useMouseInElement(target, options = {}) {
      ...
      const targetRef = vueDemi.ref(target || (window === null || window === void 0 ? void 0 : window.document.body));
  ...
  function useMousePressed(options = {}) {
      ...
      const target = vueDemi.ref(options.target);
  ...
  function useMutationObserver(el, callback, options = {}) {
      ...
      const stopWatch = vueDemi.watch(elRef, (newEl) => {
          cleanup();
  ...
  function useTimestamp(options = {}) {
      ...
      const controls = useRafFn(() => ts.value = shared.timestamp() + offset, { immediate: true });
  ...
  function useTransition(source, options = {}) {
      ...
      const getValue = shared.isFunction(transition)
          ? transition
      ...
      const { resume, pause } = useRafFn(() => {
          const now = Date.now();
          const progress = shared.clamp(1 - ((endAt - now) / duration), 0, 1);
  ...
  function useVModel(props, key, emit) {
      var _a;
      const vueCurrentInstance = vueDemi.getCurrentInstance();
      // @ts-expect-error mis-alignment with @vue/composition-api
      const _emit = emit || vueDemi.isVue3 ? vueCurrentInstance === null || vueCurrentInstance === void 0 ? void 0 : vueCurrentInstance.emit : (_a = vueCurrentInstance === null || vueCurrentInstance === void 0 ? void 0 : vueCurrentInstance.$emit) === null || _a === void 0 ? void 0 : _a.bind(vueCurrentInstance);
      return vueDemi.computed({
          get() {
              return props[key];
          },
  ...
  function useWebSocket(url) {
      const data = vueDemi.ref(null);
      const state = vueDemi.ref('CONNECTING');
      let ws;
  ...
  const useWebWorkerFn = (fn, { dependencies = [], timeout, window = defaultWindow, } = {}) => {
      const worker = vueDemi.ref(undefined);
      const workerStatus = vueDemi.ref('PENDING');
      const promise = vueDemi.ref({});
      const timeoutId = vueDemi.ref(undefined);
